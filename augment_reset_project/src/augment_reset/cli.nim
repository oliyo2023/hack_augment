...
proc showHelp*() =
  ...
  echo "  -i, --interactive  交互式选择清理目标"
  echo "  --no-backup     跳过备份（不推荐）"
  echo "  --verbose       显示详细输出"
  echo ""
  echo "示例:"
  echo "  augment_reset                    # 清理所有"
  echo "  augment_reset --vscode           # 仅清理 VS Code"
  echo "  augment_reset --jetbrains        # 仅清理 JetBrains IDE"
  echo "  augment_reset -i                 # 交互式选择"
